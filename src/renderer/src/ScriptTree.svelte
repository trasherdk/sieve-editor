<script lang="ts">
  ...
  function rowClass(row: ScriptTreeRow): string {
    ...
    const base =
      'flex w-full min-w-0 items-center gap-1 px-2 py-0 text-left text-sm leading-none'
...
{#each rows as row (row.id)}
  ...
  {:else}
    ...
    >
      <span class="shrink-0 font-mono text-sm leading-none text-zinc-500 whitespace-pre">{row.gutter}</span>
      <span class="min-w-0 truncate {row.kind === 'missing' ? 'text-bad' : ''}">{row.name}</span>
...
{#if unused.length}
  ...
  {#each unused as name (name)}
    <button
      class="block w-full truncate px-3 py-0.5 text-left text-sm leading-5 hover:bg-ink {currentName === name
        ? 'bg-ink text-accent'
        : ''}"
      onclick={() => onopen(name)}
    >
